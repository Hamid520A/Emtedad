...
import { ArrowRight, Save, Image as ImageIcon, FileText, Trophy, Settings, CalendarClock } from 'lucide-react';
...
export default function CreateContestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalData = {
      ...formData,
      // اگر وضعیت active بود، تاریخ شروع را همین الان در نظر می‌گیرد
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#faf9f6] pb-24 font-sans text-[#1a2e44]" dir="rtl">
      ...
      <form onSubmit={handleSubmit} className="px-6 space-y-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 space-y-5">
          ...
          <div>
            ...
            <input 
              type="text" 
              required
              className="w-full p-4 bg-[#faf9f6] border-none rounded-2xl text-[#1a2e44] focus:ring-2 focus:ring-[#c5a059] outline-none transition-all font-bold"
              placeholder="مثلاً: مسابقه هوش مصنوعی"
          ...
          <div>
            ...
            <textarea 
              rows={3}
              className="w-full p-4 bg-[#faf9f6] border-none rounded-2xl text-[#1a2e44] focus:ring-2 focus:ring-[#c5a059] outline-none transition-all font-medium text-sm leading-relaxed"
              placeholder="توضیحات و قوانین شرکت در این مسابقه را بنویسید..."
          ...
          {/* فیلد زمان که فقط برای حالت "به زودی" رندر می‌شود */}
          {formData.status === 'upcoming' && (
            <div className="transition-all duration-300 animate-in fade-in slide-in-from-top-2">
              <label className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                <CalendarClock size={14} /> زمان شروع رقابت
              </label>
              <input 
                type="datetime-local" 
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 space-y-5">
          ...
          <div>
            ...
            <div className="relative group">
              ...
              />
              {formData.image_url && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-green-500 bg-green-50 px-2 py-1 rounded-md">آپلود شد ✅</span>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative group">
              ...
              />
              {formData.file_url && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-green-500 bg-green-50 px-2 py-1 rounded-md">آپلود شد ✅</span>}
            </div>
          </div>
        </div>
        ...
        <button 
          type="submit"
          className="w-full bg-[#1a2e44] text-white p-5 rounded-[2rem] font-black flex items-center justify-center gap-3 hover:bg-[#2a405a] transition-all shadow-xl shadow-blue-900/10 active:scale-95 mt-4"
        >
          <Save size={22} className="text-[#c5a059]" /> 
          ذخیره و انتشار مسابقه
        </button>
