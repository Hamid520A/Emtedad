    ...
    <div className="min-h-screen bg-[#faf9f6] dark:bg-[#182234] dark:bg-[#182234] dark:bg-[#0b0f19] text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 dark:text-slate-100 font-sans pb-10" dir="rtl">
      <header className="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 dark:text-slate-400 dark:text-slate-400 text-sm font-bold mt-1">مشاهده مشخصات فردی، کدملی و وضعیت آخرین آزمون کاربران</p>
          </div>
        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-40 flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#182234] dark:bg-[#182234] rounded-[2.5rem] w-full max-w-3xl shadow-2xl border border-gray-100 dark:border-slate-800 dark:border-slate-800 max-h-[85vh] overflow-y-auto flex flex-col animate-in zoom-in-95 duration-200 text-right">
            ...
            <div className="p-6 border-b border-gray-100 dark:border-slate-800 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white dark:bg-[#182234] dark:bg-[#182234] z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 dark:text-slate-400 dark:text-slate-400 font-bold mt-0.5">مشخصات هویتی و تاریخچه کامل حضور در آزمون‌ها</p>
                </div>
              </div>
                    ...
                    {isEditing ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input type="text" required className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.first_name} onChange={(e)=>setEditFormData({...editFormData, first_name: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">نام خانوادگی</label>
                          <input type="text" required className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.last_name} onChange={(e)=>setEditFormData({...editFormData, last_name: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">شماره تماس</label>
                          <input type="text" required className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-left font-mono text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.phone} onChange={(e)=>setEditFormData({...editFormData, phone: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">کد ملی</label>
                          <input type="text" required className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-left font-mono text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.national_id} onChange={(e)=>setEditFormData({...editFormData, national_id: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">استان</label>
                          <input type="text" required className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.province} onChange={(e)=>setEditFormData({...editFormData, province: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">شهر</label>
                          <input type="text" className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.city} onChange={(e)=>setEditFormData({...editFormData, city: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">تاریخ تولد</label>
                          <input type="text" className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" placeholder="مثال: ۱۳۸۰/۰۱/۱۵" value={editFormData.birth_date} onChange={(e)=>setEditFormData({...editFormData, birth_date: e.target.value})} />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 dark:text-slate-400 dark:text-slate-400 mb-1">جنسیت</label>
                          <select className="w-full p-2.5 bg-white dark:bg-[#182234] dark:bg-[#182234] border border-gray-200 dark:border-slate-800 dark:border-slate-800 rounded-xl text-xs font-bold text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 outline-none focus:ring-1 focus:ring-[#c5a059]" value={editFormData.gender} onChange={(e)=>setEditFormData({...editFormData, gender: e.target.value})}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div className="flex items-center gap-2.5 text-xs">
                          ...
                          <div><span className="block text-[9px] text-gray-400 dark:text-slate-400 dark:text-slate-400 font-bold mb-0.5">شماره تماس</span><span className="font-black font-mono text-gray-700">{selectedUser.phone}</span></div>
                        </div>
                        <div className="flex items-center gap-2.5 text-xs">
                        ...
                        <div className="flex items-center gap-2.5 text-xs">
                          ...
                          <div><span className="block text-[9px] text-gray-400 dark:text-slate-400 dark:text-slate-400 font-bold mb-0.5">جنسیت</span><span className="font-black text-gray-700">{selectedUser.gender === 'male' ? 'مرد' : selectedUser.gender === 'female' ? 'زن' : 'نامشخص'}</span></div>
                        </div>
                        <div className="flex items-center gap-2.5 text-xs">
                          <ShieldCheck size={16} className={selectedUser.is_admin ? "text-purple-500" : "text-gray-400 dark:text-slate-400 dark:text-slate-400"} />
                          <div>
                            <span className="block text-[9px] text-gray-400 dark:text-slate-400 dark:text-slate-400 font-bold mb-0.5">وضعیت کاربری</span>
                        ...
                        <table className="w-full text-right text-xs">
                          ...
                          <tbody className="divide-y divide-gray-50 font-bold text-gray-600 dark:text-slate-300 dark:text-slate-300">
                            {selectedUser.history.map((h: any, idx: number) => (
                              ...
                              >
                                <td className="p-3 text-[#1a2e44] dark:text-slate-100 dark:text-slate-100 font-black group-hover:text-[#c5a059]">{h.contest_title}</td>
